...
void setup_malsite_lst(void)
{
    struct malsite temp, *node;
    FILE *malsitelst_fp;

    malsitelst_fp = fopen("mal_site.txt", "rt");

    while(fscanf(malsitelst_fp, "%s", temp.name) != -1) {
        node = (struct malsite *)malloc(sizeof(struct malsite));
        memcpy(node->name, temp.name, strlen(temp.name));
        node->next = NULL;
        if(!head) {
            head = node;
            tail = node;
...
int is_malsite_req(u_char *http_packet, int tcp_payload_size)
{
    FILE *log_fp;
    struct malsite *node;
    ...
    token = strtok(http_payload, " :\r\n");
    if(!token) { free(http_payload); return 0; }

    if(!strcmp("GET", (char *)token)) {
        token = strtok(NULL, " :\r\n");
        while(token) {
    ...
    while( 1 ) {
        ...
        if(ntohs(eth_hdr->ether_type) == ETHERTYPE_ARP) {
            ...
            if(!memcmp(eth_hdr->ether_shost, sender_mac, ETH_ALEN)) {
                tcp_payload_size = ntohs(ip_hdr->ip_len) - ip_hdr->ip_hl * 4 - tcp_hdr->doff * 4;
                if(is_malsite_req((u_char *)tcp_hdr + tcp_hdr->doff * 4, tcp_payload_size)) continue;
